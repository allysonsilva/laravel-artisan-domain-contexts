...
class FreshCommandTest extends MigrateCommandTestCase
{
    ...
    public function runningFreshFromAllContexts(): void
    {
        $this->artisan($this->commandName)
             ->expectsOutput(Mockery::pattern('/Dropping all tables/'))
             ->expectsOutput(Mockery::pattern('/Preparing database/'))
             ->expectsOutput(Mockery::pattern('/Creating migration table/'))
